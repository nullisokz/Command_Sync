...
{
    public record CreateUserDTO(string name, string password, bool admin);
    public static async Task<Results<Ok<int>, BadRequest<string>>> CreateUser(CreateUserDTO user, NpgsqlDataSource db)
    {
        try
        {
            ...
            string hashedPassword = hasher.HashPassword(new object(),user.password);
            ...
            cmd.CommandText=
                                ...
                                VALUES
                                (@name, @password_hash, @isadmin)
                                RETURNING id
                                """;
            cmd.Parameters.AddWithValue("@name", user.name);
            cmd.Parameters.AddWithValue("@password_hash", hashedPassword);
            cmd.Parameters.AddWithValue("@isadmin", user.admin ? true : false);
